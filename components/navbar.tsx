    ...
    return (
        <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${isScrolled ? 'bg-white/90 backdrop-blur-md border-b border-gray-200 py-4' : 'bg-transparent py-6'}`}>
            <div className="container mx-auto px-6 flex justify-between items-center">
                <div className="font-mono font-bold text-xl tracking-tight flex items-center gap-2 cursor-pointer hover:scale-105 transition-transform text-brand-black">
                    <div className="w-2.5 h-2.5 bg-brand-accent rounded-full" />
                    datatrace
                </div>

                {/* Desktop Menu */}
                <div className="hidden md:flex items-center gap-8 font-mono text-[11px] font-medium uppercase tracking-wider text-gray-500">
                    {links.map((item) => (
                        <a key={item.name} href={item.href} className="hover:text-brand-black transition-colors relative group">
                            {item.name}
                ...
                {isOpen && (
                    ...
                    >
                        <div className="p-6 flex flex-col gap-5">
                            {links.map((item) => (
                                <a key={item.name} href={item.href} onClick={() => setIsOpen(false)} className="font-mono text-sm font-medium text-brand-black hover:text-brand-accent transition-colors">
                                    {item.name}
                                </a>
                            ))}
                        </div>
